...
2. 登录后自动常驻运行 `watch-portproxy.ps1`（双守护）：
   - **代理守护**：检测 `127.0.0.1:8443` 无监听时自动拉起 `proxy.js`
   - **转发守护**：每 30 秒检测，发现"网卡在线但 443 未监听"即自动重建转发（支持蒲公英/Tailscale 双 IP）
3. 状态变化写入 `watch-portproxy.log`，便于排查

手动验证：蒲公英重连后 30-60 秒内转发自动恢复，代理崩溃后 5 秒内自动拉起，无需干预。
